...
expression} and \term{revision control}---which will be covered in sections
\ref{sec:regexs} and \ref{sec:revisions}---, as well as user modules that
extend the base functionality.  Advanced \acronym{GUI} text editors include
\inx{Sublime Text}, \inx{Atom}, and \inx{PSPad}. Advanced \acronym{CLI} text
editors include \acronym{emacs}, \acronym{vi}, and \acronym{vim}. The presented
\acronym{CLI} text editors are especially notorious for their learning curve,
whose steepness is only matched by the power these editors grant to those who
wield them.
...
expression} are a more intuitive reformulation of regular grammars that can be
used to find and replace character strings within text.

\subsubsection{Syntaxes}
% POSIX basic, POSIX extended and Perl
...
\section{Revision Control}\label{sec:revisions}
