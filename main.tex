...
Each chapter contains numerous examples, which illustrate the subject matter. In
most cases, these examples are removed from the flow of the main text and aren't
necessary for its comprehension. Although care was taken not to favour any
computing environment, some examples feature Unix utilities, which have no
suitable counterpart in operating systems such as Windows. To try these examples
out, the reader is advised to install a Unix-like environment, such as the
Windows Cygwin, on their local machine or to use a Unix-like operating system,
such as \Linux.
...
layouts that will allow the user to input any \acronym{UCS} character. Some text
editors and word processors may provide additional input methods of their own.
...
\subsection{Text Editors and Word Processors}
...
\acronym{joe}, \acroshort{GNU} nano\acroindex[!nano]{GNU}, and \acronym{pico}.

More advanced text editors come with the support for \termpl{regular
expression}, \term{revision control}---both of which will be covered in the
following sections---, and user modules that extend the base functionality.
Advanced \acronym{GUI} text editors include \inx{Sublime Text}, \inx{Atom}, and
\inx{Brackets}. Advanced \acronym{CLI} text editors include \acronym{emacs},
\acronym{vi}, and \acronym{vim}. The presented \acronym{CLI} text editors are
especially notorious for their learning curve, whose steepness is only matched
by the power these editors grant to the user.

Word processors are a breed of text editors that produces fully-formatted text
documents instead of (or along with) text files.
% ...
A reader who wishes to use a word processor instead of a text editor is advised
to only no more than the logical markup capabilities of the word processor
during the writing stage of the document preparation process.

% Word processors (Word, TextEdit, WordPad)
\subsection{Regular Expressions}
%%% Does (should) LC_COLLATE affect character ranges?
%%%   <http://unix.stackexchange.com/a/16028/42056>
...
breaking thoughts into fragmented strings of words, which the markup reassembles
...
One of the archetypal batch-oriented systems are \cliutil*{troff}%
...
\cliutil*{troff} and its derivatives, the language of \TeX\ is ripe with
...
classes with pure logical markup is \LaTeX\index{LaTeX@\LaTeX}: the standard
markup language for academic and technical documents.
