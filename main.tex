...
\begin{figure}
  ...
  \def\pc{\incgr{pc}} \def\db{\incgr{db}} \def\bend{15}
  \newlength\radius
  \setlength\radius{.18\textwidth}
  \newlength\clientposabove
  \setlength\clientposabove{.1\textwidth}
  \newlength\serverposabove
  \setlength\serverposabove{.01\textwidth}
  \def\sqrttwo{1.2}
  \tikzset{>=latex}
  \footnotesize
  \begin{tikzpicture}
    ...
    \path (A) edge [loop above, in=70, out=110, min distance=.05\textwidth, ->]
              node[above=\serverposabove] {\command{svnadmin create}} (A)
  ...
  \end{tikzpicture}\hfill\begin{tikzpicture}
    ...
    \path (B) edge[red,bend left=\bend] (A) (A) edge[red,bend left=\bend, ->] (B)
          (C) edge[red,bend left=\bend] (A) (A) edge[red,bend left=\bend, ->]
              node[red,rotate=45,above=\clientposabove]
                {\command{svn update}} (C)
          (D) edge[red,bend left=\bend]
              node[blue,rotate=-45,above=\clientposabove]
                {\command{svn commit}} (A) (A) edge[red,bend left=\bend, ->] (D)
          (B) edge[blue, ->] (A) (C) edge[blue, ->] (A) (D) edge[blue, ->] (A);
  \end{tikzpicture}}
  \caption{The basic \acroshort{SVN} workflow. After a remote repository has been
    established, the users download the latest version of the document and then
    keep downloading the latest version and uploading their changes.}
\end{figure}
