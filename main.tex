...
local machine or to use a Unix or Unix-like operating system, such as \Linux.
...
More comprehensive style guides---such as the \work{Chicago Manual of Style} or
the \work{Oxford Style Manual}\footnote{
...
regular expressions on \Unices. Both syntaxes are described in Table
\ref{tab:regexs}. More extensive syntaxes include the \acroshort{GNU} extensions
of \acronym{BRE} and \acronym{ERE}, the one used in the \inx{Perl} programming
language, and their dialects and derivatives. For most of these extended
syntaxes, the term \term*{regular} is a misnomer, as the expressions can be used
to build grammars that, according to the Chomsky hierarchy, aren't
regular.\footnote{
...
server. An example of centralized \acronym{VCS} is \acronym{SVN}\footnote{
  ...
  book}\index{Subversion book|see{\work*{Version Control With Subversion}}},
...
Some of the existing markup is directly tied with specific \acroplfst{DPS}.
