...
Berner, a computer scientist who worked at \acronym{IBM} during 1956--1962 and
who drafted \acronym{ASCII}---the digital encoding of text, standardized in
\cite{asa63}, that unified the industry and enabled computer networking on large
scale.

\subsubsection{ASCII}
In \acronym{ASCII}, every character is represented by a sequence of seven bits,
called \term{a \inx{character code}}. This allows for 128 combinations, which
are used to encode \term{\inx{printable character}s}---spanning the letters of
the English alphabet, digits, punctuation, and other symbols---and
\term{\inx{control code}s}, as depicted in Table \ref{tab:ascii}. Unlike
...
control codes, is human-readable and unlimited in its extensibility.
  ...
  % The ASCII table (both the original and ANSI X3.4-1986:
  % <http://sliderule.mraiow.com/w/images/7/73/ASCII.pdf>)

  % Some of the ASCII predecessors and contenders):
  % ASCII, ISO/IEC 646, EBDIC, Baudot Code, Morse Code
...
were based on \acronym{ASCII} and utilized the additional bit to encode
...
\begin{itemize}
  \item Each character is exactly eight bits wide. The buffering and measuring
    of character strings is therefore as simple as with \acronym{ASCII}.
...
creation of Unicode \cite{unicode91,unicode92} and \acronym{UCS} \cite{iso93} in
...
% remove direct references to bibliography (in \cite{...})

\Acronym{UCS} is an ever-expanding catalogue of characters ranging from writing
...
1,114,111 (\hexa{10FFFF} in hexadecimal) with the codes of the most common
characters falling inside the range of 0--65,535 (\hexa{FFFF} in hexadecimal).
Encodings of \acronym{UCS} map character numbers to binary character codes. ???
such encodings are specified in \cite{iso93}

  % A table explaining the UTF-8 encoding
...
for sharing linked documents within \acronym{CERN}. The system laid foundation
for today's \term{\inx{World Wide Web}} (Web) and earned its author knighthood.
The markup language used to write documents for the system was an application of
\acronym{SGML} called \acronym{HTML}. In 1993, the Web started to gain
...
1994, Timothy John Berners-Lee formed \acronym{W3C}, which has since been
