...
spoken words and written ideographs needs to be memorized by the writer on a
word-to-word basis. Some languages, such as Czech, use vastly different grammar
rules for speaking and for writing, which means that a spoken sentence needs to
be translated first before writing down. These specifics need to be recognized
by the writer.
...
\term{text processing} and will be the focus of this section.
...
during 1956--1962 and who drafted \acronym{ASCII}---a \term{character
encoding} from \citeyear{asa63} \cite{asa63} that unified the industry and
...
(\hexa{FFFF}) called \acronym{BMP}.  \Acronym{UCS} encodings map character
numbers to binary character codes. Three major encodings are specified in the
...
\begin{description}
  ...
  \item[\acroshort{UTF}-8]\acroindex[!UTF-8@{\acroshort{UTF}-8}]{UTF}
    ...
    \acronym{ASCII}, any text encoded in eight-bit \acroshort{ASCII} is
...
\acroshort{UTF}-32 is primarily used\footnote{
  ...
  Unicode support to protocols originally intended for the seven-bit
  \acroshort{ASCII}, such as e-mail.}
...
compromise between the simplicity of \acronym{ASCII} and the heterogeneity of
eight-bit encodings:
\begin{itemize}
  ...
  \item \Acronym{BOM} with the code point of 65,279 (\hexa{FEFF}) can be
    inserted at the beginning of a text as a signature of Unicode encodings. As
    the name suggests, the order in which the \hexa{FE} and \hexa{FF} bytes
    arrive also indicates the integer byte order (called \term{endianity}).
    In \acroshort{UTF}-32 and \acroshort{UTF}-16, the byte order can be chosen
...
To this end, there exists a wealth of \termpl*{markup language} that enable the
enrichment of text with additional information and labels. Aside from
\term*{logical markup}, which captures the logical structure of the document,
...
means of ensuring consistency that should be used whenever appropriate.
\index{markup!logical|)}\index{markup!presentation|)}
...
\citeyear{goldfarb81} \cite{goldfarb81} and finally standardized as \acronym{SGML}
\footnote{
...
\citeyear{bray98} \cite{bray98}. Along with the introduction of \acronym{XML},
the \acroshort{SGML} specification received a technical corrigendum
...
\acroindex[!namespace]{XML}, which were added to the specification in
\citeyear{bray99} \cite{bray99}. Namespaces enable the inclusion of elements and
...
In \citeyear{bernerslee89} \cite{bernerslee89}, an English computer scientist
\person{Timothy John Berners-Lee} proposed a decentralized system for sharing
linked documents within \acronym{CERN}. The system laid foundation for the Web
...
\citeyear{bernerslee95} \cite{bernerslee95}. As the Web was becoming ubiquitous,
it began accumulating an increasing number of documents that weren't valid
instances of \acronym{HTML}, since most Web browsers, when faced with a
malformed document, would act in accordance with the Postel's law and try to
render the document despite its deficiencies. In an attempt to unify the way
malformed \acronym{HTML} documents were rendered across the Web browsers,
\acronym{W3C} acknowledged and documented this behaviour as a part of the
\acroshort{HTML}5 specification \cite[sec.~8.2]{hickson14}. An example is
presented in Figure \ref{fig:overlapping-elements}.
...
\acronym{CSS}, which was introduced by \acronym{W3C} in \citeyear{lie96}
\cite{lie96}. The language enabled the specification of the visual properties of
any element, which allowed for the separation of document markup and design,
effectively eliminating the need for the presentation markup.
...
\cite{netscape95} complement Java applets -- a role it has since outgrown.
Standardized in \citeyear{ecma97} \cite{ecma97}, JavaScript blurs the line
between static documents and interactive applications and remains the
...
The idea was brought to fruition \cite{pemberton00} as an \acronym{XML}
...
created \acronym{RDF} \cite{lassira99}, a method for the description of
...
\acronym{OWL} \cite{mcguinness04}, also exist.

The syntax of \acronym{RDF} is not fixed, meaning that, beside the
\acroshort{XML} serialization \cite{lassira99}, other languages, such as
\acronym{JSON-LD} \cite{sporny14}, the \inx{Turtle} language
\cite{beckett14:turtle}, or the line-based \inx{N-Triples} language
\cite{beckett14:nt}, can be used to represent an \acroshort{RDF} document. A
noteworthy serialization of \acronym{RDF} is \acronym{RDFa} \cite{adida08}.
Although various serializations of \acronym{RDF} can be included in or linked to
an \acroshort{HTML} or \acroshort{XHTML} document, this will often result in an
...
intended to gradually obsolete the loosely-defined use of \acronym{CSS} class
names, attributes, the \element{meta} element, and the \element{link} element to
encode additional application-specific metadata about the document---a technique
known as \term{microformatting}.
...
systems. These can be dichotomized into \termpl*{batch-oriented system}
...
demand, and \termpl*{interactive system} \index{document preparation
