...
declares the syntax of an \acroshort{SGML} document and the semantics of the
...
visual properties of an \acroshort{XML} document. While some of these languages
may not be \acronym{XML} languages, they are nevertheless used within documents
of various \acronym{XML} formats and form an important part of the
\acroshort{XML} ecosystem.
...
\acronym{SGML} feature of \inx{\texttt{CONCUR}}, which makes it possible to mark
up several structural views of a single document.  Unlike with \texttt{CONCUR},
which ties each view to an \acroshort{SGML} \acronym{DTD}, there exists no
general mechanism for the translation of \acropl{IRI} to \acronym{XML} schemata.
This makes it impossible to validate namespaced \acronym{XML} documents, unless
all used \acropl{IRI} and their respective schemata are known to the parser.

\begin{figure}[hb!]
  \input{examples/02/peer-gynt.tex}
  \caption{The markup of the dramatic and metrical views of the beginning of
    Henrik Ibsen's \work{Peer Gynt} using the \texttt{CONCUR} feature of
    \acronym{SGML}}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{\acronym{XML} documents can be easily validated against \acronym{XML}
    schemata using command-line tools, such as \inx{\texttt{xmllint}}.}
\end{figure}
...
today. In 1994, Timothy John Berners-Lee formed \acronym{W3C}, which has since
...
<b>This text is bold, </b><i><b>bold and italic</b>, italic.</i>
  ...
  \caption{The fragment on line 1 contains overlapping elements and as such, it
    can't be a part of a valid \acronym{HTML} document.  However, it is
...
\acronym{XHTML} succeeded in fulfilling this vision for human-readable
...
\acronym{W3C} released \cite{lassira99} containing the specification of
\acronym{RDF}, a method for the description of resources on the Web.
...
predicate}\acroindex[!predicate]{RDF}, \term{a subject}\acroindex[!subject]{RDF}
and \term{an object}\acroindex[!object]{RDF}, where both the predicate and the
subject are specified as \term{resources} \acroindex[!resource]{RDF} using
\acropl{IRI}. If the object of a triplet $(p,s,o)$ is also a resource, the
triplet can be interpreted as a subject $s$ being in a relation $p$ with the
object $o$. If the object is a \term{literal value} \acroindex[!literal]{RDF}
...
\acronym{RDF} is \acronym{RDFa} specified within \cite{adida08}. While various
...
One of the archetypal batch-oriented systems are \inx{\texttt{troff}} and
\index{\texttt{nroff}|seealso{\texttt{troff}}} \texttt{nroff} from 1970s, which
...
alternative from \acronym{GNU} called \texttt{groff}
\index{\texttt{groff}|seealso{\texttt{troff}}} combines the capabilities of both
...
packages, such as \inx{\texttt{man}} for the formatting of documentation,
\texttt{me} \index{\texttt{troff}!\texttt{me}} for the creation of research
papers, or the more recent \texttt{mom} \index{\texttt{troff}!\texttt{mom}} for
...
\begin{figure}
  ...
  \caption{An excerpt from the beginning of Edgar Allen Poe's \work{Cask of
    Amontillado} formatted using the \texttt{mom}
    \index{\texttt{troff}!\texttt{mom}} macro package of \inx{\texttt{groff}}.}
  \label{fig:poe}
\end{figure}
...
Programming}, and found the appearance of mathematical formulae unsatisfactory.
...
markup languages} \index{lightweight markup language} leverage symbols and
...
and write. Examples of lightweight markup languages include \inx{Markdown},
