...
possible.\index{markup!logical|)}\index{markup!presentation|)}
...
\begin{figure}
  \ffootnote{\acronym{DTD}s in \acronym{SGML} and \acronym{XML} documents can be
...
include DocBook\acroindex[!DocBook]{XML}\footnote{
...
\begin{figure}[b]
  ...
  \caption{Line 1 contains overlapping elements and, as such, can't be a part of
    a valid \acronym{HTML} document. Nevertheless, it is recommended by
    \acronym{W3C} that browsers handle it identically to line 2.}
...
navigated by software agents.\footnote{
  ...
  \person{\citeauthor{bernerslee06}} in \citeyear{bernerslee06} in the article
...
\cliutil*{groff}\index{groff@\cliutil*{groff}|see{\cliutil*{troff}}}, which was
...
markup language of \cliutil*{groff} combines presentation markup with
programming constructs and enables the definition of logical markup through user
macros. The standard macro packages for \cliutil*{groff} include
\identifier{man} for the \index{troff@\cliutil*{troff}!\identifier{man}}
formatting of documentation, \identifier{me}
\index{troff@\cliutil*{troff}!\identifier{me}} for the creation of research
papers, and the more recent \identifier{mom}
...
\begin{figure}
  ...
  \caption{An excerpt from the beginning of \person{Edgar Allen Poe}'s
    \work*{Cask of Amontillado}\workindex{the Cask of Amontillado} formatted
      through the \identifier{mom} macro package of \cliutil*{groff}. The
      markup was borrowed from the web page of \identifier{mom}%
      ~\cite{schaffter15}.}
  \label{fig:poe}
\end{figure}
