...
\begin{figure}
  ...
  \caption{A chromacity diagram, where the shark-fin-shaped \term{spectral locus}
    corresponds to a narrow-band \acronym{SPD} with power at just a single
...
\termpl*[primaries]{primary}, and the \term{white point}. The extent of the
colors that can be represented by an $RGB$ color space (its \term{gamut}) can
...
profile}, which fully describes the $RGB$ color space.

Mixing cyan, magenta, and yellow inks gives rise to the subtractive
\term*{$CMYK$ color}\index{color model!cmky} model. Although three colors ($C,
