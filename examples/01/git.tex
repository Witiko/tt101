...
\end{tikzpicture}\hfill\begin{tikzpicture}
  ...
  \node[\gitcommit, below of=H, node distance=\clientposbelow]
    {\command{git commit}};
\end{tikzpicture}
\caption{The basic Git workflow. After a remote repository has been
  established, users make a local copy of the entire repository and then
  \textcolor{\gitcommit}{store changes in their local repositories} or
  \textcolor{\gitcheckout}{revert changes from their local repositories}. Every
  once in a while, users \textcolor{\gitpull}{download the latest changes by
  other users} and \textcolor{\gitpush}{upload changes of their own} in bulk.}}
